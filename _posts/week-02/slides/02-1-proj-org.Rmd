...
- One dir hierarchy for one project &mdash; in other words:
  - Don't keep multiple distinct projects under one dir.
...
- Possible to use *relative paths* within a project,  
  such that your paths in scripts are portable


<p align="center">
<img src=img/proj-dirs-1.svg width=60%>
</p>
...
<p align="center">
<img src=img/proj-dirs-2.svg width=70%>
</p>
...
- **Relative paths** start from a *working dir*: when you alway use the root of
  the *project* as the working dir, paths work when moving the project and
  across computers.
]
...
<p align="center">
<img src=img/proj-dirs-3.svg width=70%>
</p>
...
<p align="center">
<img src=img/proj-dirs-6.svg width=80%>
</p>

--

For **data**, something similar can be applied once the data have been  
...
<p align="left">
<img src=img/proj-ex-comb.png width=85%>
</p>
...
.content-box-info[
**Several things depend on preference and project specifics:**
- Can also spread data across top-level dirs:  
- Can also use `analysis` dir instead of `results` (Buffalo)
- People writing software often use a `src` (source) dir for code.
...
- In file names, provide *metadata* like Sample ID, date, treatment:
  ...
  - `samples_soil_trtA_2019-01.txt`

- Now, you can use globbing to select samples from a certain month or treatment:
  ```sh
  $ ls *2016-05*
...
- *One* good way (opinionated recommendation):
  ...
  - Use **`_`** to delimit units you may want to separate on
    (extract with `cut`, etc) later: sampleID, batch, treatment, date, etc. 
  
  - Within such units, use **`-`** to delimit words: `grass-samples`.
  
  - Reserve periods to use for the file extension.
...
- Can group similar files together by starting with same phrase,  
  and number scripts according to execution order:
  ```sh
  DE-01_normalize.R
  DE-02_test.R
