...
## `r icon::fa("user-edit")` Quoting variables: Your turn

1. Create a variable whose value has spaces in it.  
   `echo` the variable to make sure it worked.

2. Do `touch <variable>` (e.g. `touch $myvar`) and then `ls` to see what happened.

3. Now *quote the variable name* in a new `touch` command,
   and then `ls` to see what happened.
   
4. Optional: clean up &ndash; `rm` the files you just created.

---

## `r icon::fa("user-edit")` Quoting variables: Solutions

1. Create a variable whose value has spaces in it and `echo`.
...
2. Do `touch <variable>` and then `ls` to see what happened.
...
3. If it is not (`else` block), also report the outcome with `echo`.
