...
- Another Jupyter peculiarity is that it will automatically start saving any
  new notebook as `Untitled.ipynb`, so get used to your directories being
  littered with `Untitled784.ipynb` files...
  
  But we will want to save the notebook under a useful name, so click  
  `File` => `Save Notebook As` => `week08/Tuesday.ipynb`.
...
- In the next cell, which is a code cell by default,
  type `30 * 2` and again press <kbd>Shift</kbd>+<kbd>Enter</kbd> to *run* the cell.
...
For us, that also has the advantage of not having to switch platforms constantly.
...
- "Commands" sidebar has all the different commands and their keyboard shortcuts. 

---

## More Jupyter tricks -> Move to week 2
