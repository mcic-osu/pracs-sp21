...
## Perl versus Python <br> to calculate the average of a series of numbers
...
but if you're not a fan, feel free to explore other environments
...
We will see an examples *mutable* objects on Thursday, such as *lists*.
]
...
- We can combine variables of the same type using operators like `+` and `*`.
  We already saw this for numbers, but we can do the same for string:
...
- And to a floating point number and a Boolean with `float()` and `bool()`,
  respectively.
...
- Note that quotes always denote strings,
  so numbers in quotes are interpreted as strings:
