...
and can be used interchangeably both with *module* and *package*,
...
So, they are not loaded by default, but are part of any Python installation.

For example: `csv`, `os`, `datetime`, and `re`.

---

## Installing modules

So-called "**third-party**" modules/packages/libraries do need to be installed.
...
**Let's install a couple of packages that we'll need next week  
...
conda install -y numpy pandas biopython scikit-image
```
]
...
function names: instead, you need to prepend the module's name.
...
- To load only  certain functions from a module,
  or to load certain modules from multi-module packages:
...
- Makes your programs more flexible (function arguments)!
...
## Keyword arguments with default values (cont.)
...
- When a variable has **"function scope"**, it is available only inside the function.
  This is the default behavior *inside functions*.
