...
We could write a *shebang* the way we did for Bash scripts,
directly providing the path to the interpreter,
which in this case would be `/usr/bin/python3` (vs. `/bin/bash`).
(This is also what CSB does in the example.)
...
Again like in our Bash scripts, we generally want scripts to take *arguments*.
...
Out simulate_drift.py script should take two arguments:  
the population size `N` and the odds for one allele `p`.
...
| Imported                | *The name of the script*
