...
- Get the length (number of elements) along each axis with `shape`:
...
- Get the number of dimensions (axes) with `ndim`:
...
- Masking works the same for multidimensional arrays,
  and we can also use assignments.
  Consider the following code which will convert all values in an array that
  are larger than the mean value, to the mean value:
