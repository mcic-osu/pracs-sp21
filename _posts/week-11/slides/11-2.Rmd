...
There are corresponding functions to read in Excel and JSON files, among others.
]
...
As a consequence, within each column elements are of the same data type,
but different columns can have different data types, as we have seen above.

(Another way to think of `DataFrame`s is as a special kind of diction-
ary, with each column as an item, and column names as keys.)
]
...
And `info()` gives another type of overview for all columns:
...
- First, note that we can specify columns not just with the syntax
  `df["colname"]` but also with the shorthand `df.colname`:
  ...
  which keeps the rowwise index as you can see.
  
--

- Next, we can slice the `Series` like we may expect to:
...
- But we can't slice the entire `DataFrame` the way we may expect to:
...
3. Bonus: How many "Warbler" species are in the dataset?
...
3. Bonus: How many "Warbler" species are in the dataset?
  ...
  ```python
  data[data.english.str.contains("Warbler")].shape[0]
  ```
