...
- First, note that we can specify columns not just with the syntax
  `df["colname"]` (think dictionary!), but also with the shorthand `df.colname`:
...
- Next, we can slice the `Series` like we may expect to for an array/list:
