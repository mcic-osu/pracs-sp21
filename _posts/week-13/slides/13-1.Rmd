...
- Go back to the original script and **run the whole workflow as a single job,**
  and per-sample steps are simply run 1-by-1 in a loop.
  This is too time-consuming in many cases...
...
- Go back to the original script and **run the whole workflow as a single job,**
  and proper Bash settings will at least make the workflow stop upon failure.
...
- Lots of testing for proper input for each script,  
  test the exit status of each script, etc.
...
- **Scalability**
  ...
  - Efficient execution &ndash; all dependencies are known,
    cluster submission is automated & optimized.
