...
- #### Using input/output directives &ndash; and relationships between rules
- #### Using a "rule all"
- #### Using *{input}* and *{output}* placeholders
- #### Using wildcards to generalize rules
- #### Specifying samples/input files with *glob_wildcards()*
...
First, let's create a directory for this:
```sh
...
- `data` will contain empty FASTQ files for three samples:
  `smpA`, `smpC`, `smpG`.
...
- Although it's a general workflow management tool,
  ...
  In practice, it appears to be used mostly for bioinformatics analyses.
...
- Also named after &ndash;and based on &ndash; Make, ...
...
- To actually run the Snakemake workflow that is specified in your `Snakefile`,
  ...
  # `Snakefile`, `snakefile`, `workflow/Snakefile`, `workflow/snakefile`
  $ snakemake -j1
  
  # If necessary, we can call the Snakefile by name with "-s":
  $ snakemake -s my_other_snakefile.smk -j1
  ```
...
Let's save the following in a file called `Snakefile` in the `workflow` dir:
...
rule map:
    shell: "cat res/smpA_trim.fastq | scripts/map.sh > res/smpA.bam"
```
...
#> FASTQ file data/smpA.fastq after trimming
#> AAAAAAA
```
...
and now we are stuck with an incomplete output file that we best manually
remove before it confuses us (this will happen when you have many output
files!).
...
```sh
cat res/smpA_trim.fastq | scripts/map.sh > res/smpA.bam
```

We will see about **redirecting standard error to log files** later,
but for now, we will keep out Snakefile simple and build on some other things
first.
]
...
Because **the output of `trim` is the input of `map`**,  
Snakemake will be able to tell that:
  
- `map` should be run before `trim`.
  
- `trim` should only be run if the output from `map`
  is *actually present*.
...
- Let's try that:
  ...
  Here, we also specified the **`-r` (long form: `--reason`) option**,  
  which makes Snakemake tell us why it decides to run things.
...
.footnote[
[1] Though you can always force a run, even if output files are present,  
...
First off, if we don't specify and rule or output file in our Snakemake call,  
...
- How can we efficiently get Snakemake to run both rules?
]
...
.content-box-answer[
- With no arguments, the first rule (`trim`) will be run.
  The other rule, `map`, will not because `trim` does not
  depend on it.

- If we run `snakemake -j1 map` or  
  `snakemake -j1 res/smpA.bam`,  
  Snakemake knows it *also* needs to run the rule `trim`.
]
...
it is handy to define a "pseudorule", with
**only `input` and no `output` or action**, as the first rule.

This rule is usually called `rule all`.

.content-box-q[
Can you think of what kind of files may be specified in `rule all`?
And why would this be useful?
]

--

- `rule all` typically has as its `input` all final output files,
  final tables, plots, and so on.
  
  Though more specifically, it has all files that are
  **not used as input for another rule (endpoints)** &ndash;
  some of these files may in fact be produced early on in the pipeline,
  such as MultiQC output.

--

- Using a `rule all` provides an easy way to **run the entire workflow**:  
  if we specify all workflow endpoints as input for `rule all`,
  and run Snakemake with no arguments,
  the entire workflow / all not-yet-run rules will be executed.
...
.content-box-answer[
The output of the final rule of the workflow, which is `res/smpA.bam`.
]
...
## Running and visualizing the workflow again (cont.)
...
Note that because we have to specify input and output files both in the
...
Note that the different input files are **separated by a comma**.
When there are multiple entries for a directive (e.g. `input`),
this comma is required. When there is only one, a trailing comma is still accepted.

--

.content-box-info[
For some rules above, values are on the same line, and for others,
they are on a new line with indentation.
Either works, but the latter is generally preferred &ndash;
certainly if you have multiple values.  
To save space, I often use a single line in these slides.
]
...
This is where Snakemake gets particularly powerful,
but also more complicated: we can include **wildcards** in our rule to take
care of this.
...
but note that we are nowhere explicily defining what `{smp}` should hold!
...
## Using wildcards in all rules

Next, we can use an equivalent wildcard in our other rule:
...
Snakemake will figure out it first needs to run `trim` with wildcard
value `smpA`, and then `map` with the same wildcard value.
...
OK, very nice, but we still haven't run the same rule multiple times for
...
You might be thinking now that perhaps Snakemake will detect these input files,
then infer that it can use `smpA`, `smpC`, and `smpG` as the wildcard values,
and run both rules for each sample.
...
Unfortunately, **Snakemake complains that there is no "concrete" rule.**

If we take for granted that Snakemake will not peek at possible input files to
infer wildcards, that makes sense:
it has no concrete instructions for output files to produce. 
...
Let's try that in an IPython window!

```python
from snakemake.io import expand, glob_wildcards

# Now run the code in the block above
```

---

## Snakefile version 5: <br> with wildcards and the expand() function
...
$ snakemake --dag | dot -T svg > workflow/DAGs/v5.svg 
```
...
- List them out in a *config* file and then read them into the Snakefile.<sup>[1]</sup> 
...
- We can use *globbing* to determine which input files are present,
  and from the input file names infer the sample IDs.

  We will now use this latter approach,
  for which Snakemake has another convenience function: **`glob_wildcards()`**.

.footnote[
<sup>[1]</sup> We will see config files later on.
]
...
.content-box-info[
This is stored like that (`.smp` rather than simply a list)
to also allow you to define multiple wildcards:

```python
>>> !ls seqdata
#> A_R1.fastq.gz A_R2.fastq.gz B_R1.fastq.gz B_R2.fastq.gz

>>> glob_wildcards("seqdata/{sample}_{read}.fastq.gz").sample
#> ['A', 'A', 'B', 'B']
>>> glob_wildcards("seqdata/{sample}_{read}.fastq.gz").read
#> ['R1', 'R2', 'R1', 'R2']
```
]
...
## Snakefile version 6: with glob_wildcards()
...
In general, in sequence analysis workflows, it is very common to switch between:

- File-by-file / **sample-by-sample** analysis: QC, trimming, mapping, etc.

- **All-samples-at-once** analysis: creating a read/ASV/OTU count table,
  calling variants, etc.
...
## Snakefile version 7: with an aggregation rule
...
$ snakemake --dag | dot -T svg > workflow/DAGs/v7.svg 
```
...
```sh
$ snakemake --dag | dot -T svg > workflow/DAGs/v7b.svg 
```
