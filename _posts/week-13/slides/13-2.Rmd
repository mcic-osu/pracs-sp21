...
- If we run `snakemake -j1 map` or  
  ...
  Snakemake knows it *also* needs to run `rule trim`.
]
...
You may expect that Snakemake would be able to detect these input files,  
...
infer wildcards (it works the other way around!), that makes sense:  
...
# Also try the code in the block above
```
...
Let's also try that in the IPython window:
