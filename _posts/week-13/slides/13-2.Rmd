...
- Also named after, and based on Make, a utility that runs Makefiles in which
  you specify a set of tasks to be executed.
...
You will see about **redirecting standard error to log files** in the exercises.
]
...
You may expect that Snakemake is able to detect these input files,
...
- Snakemake accepts *config* files and we can list the samples there and access
  the list from within the Snakefile.
...
- We can use *globbing* to determine which input files are present,
  and infer the sample IDs from the input file names.
