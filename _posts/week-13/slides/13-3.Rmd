...
```sh
scripts/trim.sh -d <input_dir> -i <sample_id>
```
...
```python
# THIS WON'T WORK:
# shell: "scripts/trim.sh -d res -o {smp} > {output}"
```

```python
# YES -- use "{wildcards.<wildcard-name>}":
shell: "scripts/trim.sh -d res -o {wildcards.smp} > {output}"
```

Why do we need `{wildcards.smp}` and not just `{smp}` akin to how we refer to
`{input}`?
...
such as in
[this example from the official Snakemake tutorial](https://snakemake.readthedocs.io/en/stable/tutorial/advanced.html#step-4-rule-parameters), which passes read group information to `bwa mem`:
