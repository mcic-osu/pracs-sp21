...
- Click the gear icon `r icon::fa("cog")` in bottom-left,
...
- For a nice overview of your directory structure:
  ...
  ```sh
  $ tree ../..    # "../.." is to tell tree to start two levels up
  ```
...
- You can make use of this when using shell expansion to *move* or *delete*
  ...
  $ rm sample[12]*    # Recall samples[12]* with "Alt ."
  ```
...
- Here, we're also interested in taking away write permission for ourselves
  for the raw data.
]
...
## Sidenote: "[The Unix Chainsaw](https://www.youtube.com/watch?v=sCZJblyT_XM)"
