...
- **Activate** the `cutadapt-env` environment and see if we can run Cutadapt:
  ...
  ```bash
  [<user>@pitzer-login01 ~]$ source activate cutadapt-env
  
  (cutadapt-env) [<user>@pitzer-login01 ~]$ cutadapt --version
  #> 3.2
  ```
...
- Since recently, both Pitzer and Owens use the **_SLURM_ scheduler**  
  (**S**imple **L**inux **U**tility for **R**esource **M**anagement).

<br>

.content-box-info[
Until summer 2020, OSC only used PBS &ndash; see
