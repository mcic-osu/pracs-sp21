...
- For example, you may want to use `$SLURM_CPUS_ON_NODE` in the call to
  a program inside the script. This way, you only have to specify this once
  and don't run the risk of using more cores than you have available.
  
  Additionally, if you specify the memory in the `sbatch` call, you don't need
  to hard-code the number of cores at all.
