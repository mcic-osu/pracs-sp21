...
Like in NumPy, we don't read files line-by-line but load them all at once &ndash;  
...
- Note that all NumPy functions, methods, and attributes will also work for
  Pandas `DataFrames`!
...
.content-box-info[
Pandas `DataFrame`s can be seen as a set of *aligned* 1-dimensional NumPy arrays
of equal length, with each `DataFrame` column representing an array.

Consequently, within each column, all elements have to be of the same data type,
but different columns can have different data types, as we see above.
]
...
- Next, we can slice the resulting `Series` (a 1-dimensional Pandas array)
  like we may expect to:
