...
- If you make a mistake like an unnecessary commit, or something that requires
  ...
  each commit is form, and I'm still able to figure out which commits are
  the ones that I requested.

- Points are also not subtracted for minor things like typos in commit messages.
...
6. **Create at least two commits while you work on the Markdown file.** *[0.5]*  
   ...
   Bad: "*First part of the file*" and "*Second part of the file*"
   
   Good: "*Summarized key concepts in the Git workflow*" and
...
9. **Create a directory with dummy data files.**  *[0.5]*  
   ...
   brace expansion
   (e.g. for samples numbered 1-100, or 20 samples for 2 treatments).
...
As it turns out, your adviser messed up,
...
would be to create a new Git *branch* and perform the renaming on that branch.
...
12. **Write a `for` loop to rename the files.**   *[1.5]*  
    ...
    - To replace the last part of the filename, like the extension,
      recall that you can strip shared suffices using the `basename` command.

    - Before you actually rename the files,
...
14. **Update the README file.**  
    ...
    (*No, the repo doesn't really make sense as a cohesive unit any longer,
...
18. **Push a final local edit.** *[1]*  
    To see how it is now quite easy to keep your remote repo in sync,
    and to practice with `.gitignore` files, you will do final commit and push:
    
    - Create a file `my.tmp~`.
    - Check your status.
...
19. Find the "Issue" tab on Github and open a new issue.
    In the issue text, tag `@jelmerp`.
