...
4.  Use a `wget` command to download all the files you see on the assembly
    ...
    because all the directory levels are creating paths that are too long.
    :::
...
2.  Count the number of lines in the genome sequence
    ...
    (The `fna` extension explicitly denotes a FASTA file with nucleotide
    sequences, as opposed to one with amino acid sequences, `.faa`.)
