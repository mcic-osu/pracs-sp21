...
So far, we did not work with real data in the Snakemake exercises.
In this exercise, you'll create a Snakemake workflow that processed some actual
FASTQ files.
...
The data and a project skeleton can be copied from `/fs/ess/PAS1855/data/week13/`:
...
Let's start an interactive job, because we will let Snakemake perform
a few Conda installations, and this happens as part of the main Snakemake process
(while individual jobs will be submitted as SLURM jobs).
Because we'll want more than one hour (the maximum for `sinteractive`),
we will use `srun` to request an interactive job, which means we have to add
`--pty /bin/bash`:

```sh
srun -A PAS1855 -t 120 --pty /bin/bash
```
...
- We use some generic Python code directly to build file paths,
  starting by importing the `join()` function from the `os.path` module.
  Further down, we use `join()` to build paths.
  
  We could also accomplish this simply by concatenating strings,
...
- In the `shell` directive of `rule clean`, we are have two lines of shell code
  inside Python triple-quotes.

Also of note:

- In `rule all`, we are already specifying the output files of the
  final workflow, so we won't have to change this rule in the exercises below.
  
- We will run the workflow on the cluster in the sense that each Snakemake job
  will be a SLURM job.
  Therefore, we specify `rule all` and `rule clean` as `localrules`
  (i.e., rules *not* to be submitted as SLURM jobs ),
  because they are not computationally intensive at all and the overhead of
  submitting a SLURM job and waiting for it start would just slow thing down
  for those rules.

#### Our "profile" `config.yaml` file

We use a "profile" `slurm_profile/config.yaml` file very similar to the one in
the previous Snakemake exercise:
...
Recall that this file simply contains options that we can also pass at the command
line, so we take the opportunity to also specify the maximum number of jobs
...
so we don't have to specify this at the command line when we call Snakemake.

Now, the only option we need to pass to Snakemake is `--profile` for which
we specify the *directory* in which this YAML file is found:
...
and the files we have used to specify or save Conda environments.

Now, we need some way to tell Snakemake about this config file. 
We can do so using the `configfile` directive in the Snakefile.
Then, all key-value pairs from the config file will be available in the Snakefile
...
Then, also assign the primer sequences to variables `PRIMER_F` and `PRIMER_R`,
respectively.
...
- Previously we have always run scripts in the `shell` directive,
  but in `rule fastqc`, we directly run FastQC in the `shell` directive.
...
- Because FastQC doesn't allow us to specify the output file,
  ...
  **Note that in the `shell` directive,
  ...
  Therefore, we can recall the FastQC output dir with `{FASTQC_DIR}`.
  
- In the `input` and `output` directives, we are using two wildcards:
  `{sample}` and `{read}`.

- To avoid excessive installations, we load the OSC module for FastQC.
...
- Add the `rule fastqc` with the contents as specified above.
...
- Check for the presence of output files and look at the log files.
  ...
  Note that when we submit jobs to the cluster, we will have **two** log files
  for every job. The log file that we define in the Snakefile will contain
  standard output and/or standard error from our actual command,
  whereas the SLURM log file will contain's Snakemake's standard output and
  standard error. Take a look at both.
...
Let's add a rule that runs MultiQC.
In this case, we will run a shell script that in turn runs MultiQC,
rather than running MultiQC directly in the `shell` directive like we did
with FastQC.

The main reason for this is an annoyance with installing MultiQC through Conda
(and it is not available as an OSC module): when using a YAML file or attempting
a one-step installation, Conda will fail.
Instead, we need to do a two-step installation: first, we install a Python version,
and then, we install MultiQC.
We actually also did this in week 6, but I will not assume you still have that
environment.
Therefore, the `scripts/multiqc.sh` script that `rule multiqc` calls will create
...
- In the `shell` directive, you will call the script `scripts/multiqc.sh`.
  Take a look to see what arguments it takes, and specify those.
...
- Again, we run a script rather than calling the program directly in the rule.
  ...
  The script takes four arguments: the name for the FASTQ file with forward
  reads (the name for the file with reverse reads will be inferred from that),
  the output dir, and the forward and reverse primer sequences.
...
- Get Snakemake to run `rule cutadapt` for at least one of the FASTQ files.
  Note that this will take a little while this first time around,
...
- Like with `rule fastqc`, you'll need to specify an actual output file.
...
```sh
$ snakemake --profile slurm_profile results/fastq_filter/201_R1.fastq
```
...
Additionally, we will request for an appropriate amount of memory,
...
<details><summary>Full Snakefile #7</summary>
