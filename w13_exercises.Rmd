...
-- copy the files from `/fs/ess/PAS1855/data/week13/` to do so:

```sh
cd /fs/ess/PAS1855/users/$USER
cp -r /fs/ess/PAS1855/data/week13/ .
...
- Every item in `input` and `output` is trailed by a comma.
  This may look a little strange (but think 1-element tuple!),
  yet is somewhat useful.[^2]
...
[^2]: When every line has a trailing comma,
      regardless of whether a new item will follow,
      you can always add and remove lines without paying attention to this.
...
<summary>Solution</summary>
<br>
Below, Snakemake is run with the `-p` (print shell commands) and `-q` (quiet)
...
- Pass `metadata/ref.fa` to the `map.sh` script in the `shell` directive.

- While not strictly necessary to avoid errors,
  you should include the `metadata/ref.fa` file in the `input` directive.
  
  Because you will now have multiple items for `input`, it will be good to
  name them using `myname=myfile`.

Moreover, it is good practice to define hard-coded input files like this one
clearly at the top of your script/Snakefile
(or even better, use a configuration file, as we'll see later).

</details>

<details><summary>Solution</summary>

- Assign the filename at the top:
...
- **What will happen if we now try to rerun Snakemake?**  
  (That is, without first deleting our previous results.)

<details><summary>Solution</summary>
<br>
It would not run any jobs:
...
# Maybe if we specify one of rule `map`'s output files?
snakemake -j1 res/smpA.bam 
#> Building DAG of jobs...
#> Nothing to be done.
```

Perhaps surprisingly, Snakemake does not automatically rerun the workflow
...
This is not optimal -- in the next section, we will therefore use log files.
...
-- or each job within a rule, actually.
...
- **Add a `log` directive to each rule**.
  ...
  - Then, remove all earlier workflow output and run Snakemake.
  
  - Check that you now have a log file for each job --
    though most will be empty in the absence of errors;
    only the logs for the `fastqc` rule should not be empty. 
  
<details>
<summary>Solution</summary>
<br>

- The full Snakefile with log directives added to each rule:
...
so those jobs could be run at the same time.
...
- It would make sense to run 3 jobs at the same time,
  since our workflow currently runs 3 samples,
...
- **Submit the SLURM job**.
  ...
  *Make sure to first delete the workflow's output files,
  or Snakemake won't do anything.*
...
call, and Snakemake will automatically submit a SLURM job for each of its jobs.
...
```sh
snakemake -j3 --cluster "sbatch --account=PAS1855"
```
...
- **Create the profile directory and config file as shown above,**
  **and additionally specify requests for time (in minutes) and number of CPUs**.
  ...
  Therefore, we can omit `ntasks` (and keep its default value of 1),
  since none of our individual jobs will run multiple processes.
...
`--profile` and specify our dir as in `--profile <profile_dir>`.

- **Run Snakemake with the profile.**  
  (Remove the old output first.)
...
snakemake -j3 --profile slurm_profiles
```
...
but directories in this config file.
:::
...
- Make Snakemake wait longer for output files to appear
  using the `latency-wait` option (units are in seconds; we want 30 seconds).  
...
snakemake --profile slurm_profiles
```
...
- Add `rule all` to the localrules, and run Snakemake again.
...
The `threads` directive in a Snakefile corresponds the the `--cpus-per-task`
...
number of threads.
Instead, they refer to the keys we use in our `config.yaml` -- for example,
to actually request 50,000 MB of memory for `rule heavy_stuff`,
`mem_mb` should map to the SLURM resource request:
...
- You can [look up](https://slurm.schedmd.com/sbatch.html)
  ...
  in the script.
  For the time limit specifically,
  this is `SBATCH_TIMELIMIT`,
